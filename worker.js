...
KEY RULES TO VERIFY:
1. ACTION: own_stock if in_stock rows exist with notes NOT containing "Warehouse#" (own inventory). add_to_stan if ALL in_stock rows have "Warehouse#" in notes (external warehouse — Warehouse#3, Warehouse#4, etc.) and stan_results not QUOTED. stan_quoted if ALL in_stock rows are "Warehouse#" and stan_results has QUOTED entry. msg_checking if OEM + buyer TP + at least one non-BILL-EXT row. request_tp_500 if OEM + NO buyer TP — buyers commonly say "no target" on first email; we always ask anyway. bill_handle ONLY if ALL OEM rows are BILL EXT AND buyer gave an explicit dollar TP. no_bid if nothing in any inventory. CRITICAL PRIORITY: stan_quoted and add_to_stan apply even when oem_results is ALSO present — Warehouse#3 stock with a QUOTED Stan entry takes priority over OEM EXCESS. Never override stan_quoted to request_tp just because oem_results exists.
