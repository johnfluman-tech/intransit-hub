...
async function handleEmailAgent(request, env) {
  ...
  // Improvement #7: msg_checking + all in_stock rows are WH3 + Stan has QUOTED → stan_quoted.
  // Happens when OEM EXCESS coexists with WH3 stock — AI picks the OEM (msg_checking) path
  // but the WH3 stan_quoted path should always win since it's our own stock with a real quote.
  if (decision.action === 'msg_checking' &&
      Array.isArray(in_stock_results) && in_stock_results.length > 0) {
    const allWH3msg = in_stock_results.every(r => /Warehouse#\d/i.test(r.notes || ''));
    if (allWH3msg) {
      const stanQuotedRowMsg = (stan_results || []).find(r => r.status === 'QUOTED' && r.colB);
      if (stanQuotedRowMsg) {
...
// NOTE: CF Worker IPs are blocked from authenticating to netcomponents.com.
// The login POST always redirects back to /#/account/login instead of /search.
// Steps 1-4 (CSRF + login attempt) run fine; step 2 silently fails (IP block).
// Result: { found: false, searchable: true } when the public search works,
//         null when any hard error occurs.
  ...
  if (decision.draft_body) {
    ...
    try {
      ...
      if (hasExistingDraft) {
        ...
        // Still apply labels and queue sheet ops — just don't create duplicate draft
        if (action !== 'bill_handle') {
          const FINAL_ACTIONS2 = ['msg_checking','own_stock','stan_quoted','add_to_stan','remove_oem','david_nostock','no_bid','listing_removed'];
          if (FINAL_ACTIONS2.includes(action)) {
            await gPost('/threads/' + threadId + '/modify', { addLabelIds: ['Label_166'] });
          }
        }
        if (decision.id) {
          await env.DB.prepare("UPDATE agent_decisions SET status='completed' WHERE id=?").bind(decision.id).run().catch(() => {});
        }
        return;
      }
    } catch(e) { /* continue — check failed, allow draft creation */ }
    ...
    if (decision.id && draftMsgId) {
      await env.DB.prepare("UPDATE agent_decisions SET status='completed', gmail_draft_id=? WHERE id=?")
        .bind(draftMsgId, decision.id).run().catch(() => {});
    }
  }
...
async function cronScanInbox(env) {
  ...
  const rfqQ = encodeURIComponent(
    'in:inbox (to:rfq@intransittech.com OR deliveredto:rfq@intransittech.com OR to:icsource.quotes@intransittech.com OR deliveredto:icsource.quotes@intransittech.com OR subject:rfq OR from:autosend@icsource.com OR subject:"please quote" OR subject:"request for quote" OR subject:"request for quotation" OR subject:"looking for" OR ((to:john.fluman@intransittech.com OR deliveredto:john.fluman@intransittech.com OR to:fluman@intransittech.com OR deliveredto:fluman@intransittech.com) ("quotation" OR "best price" OR "netcomponents" OR "looking for" OR "quote your stock" OR "can you quote" OR "is it in stock" OR "availability" OR "your price" OR "price and quantity" OR "purchase" OR "need to buy" OR "price each" OR "usd each" OR "pcs" OR "pieces" OR "i need"))) -from:intransittech.com -from:fortetechno.com -from:fortecomp.com -from:partalert@netcomponents.com -label:oem-rfq-incoming-processed newer_than:3d'
  );
  ...
  // Improvement #9: adaptive throttle — if subrequest errors spiked last run, process only 1 thread
  ...
  for (const { tid, source } of toProcess) {
    try {
      ...
      // Improvement #9: track subrequest errors for adaptive throttle next run
      if (e.message && e.message.includes('Too many subrequests')) {
        try {
          const curRow = await env.DB.prepare("SELECT value FROM rules WHERE type='cron_state' AND key='subrequest_error_count'").first();
          const newCount = (curRow ? parseInt(curRow.value) || 0 : 0) + 1;
          if (curRow) {
            await env.DB.prepare("UPDATE rules SET value=? WHERE type='cron_state' AND key='subrequest_error_count'").bind(String(newCount)).run();
          } else {
            await env.DB.prepare("INSERT INTO rules (type, key, value) VALUES ('cron_state', 'subrequest_error_count', ?)").bind(String(newCount)).run();
          }
        } catch(e2) {}
      }
  ...
  if (subreqErrorCount > 0) {
    await env.DB.prepare("UPDATE rules SET value='0' WHERE type='cron_state' AND key='subrequest_error_count'").run().catch(() => {});
  }
}
